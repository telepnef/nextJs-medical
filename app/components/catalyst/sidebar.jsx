...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  let classes = clsx(
    // Base
    "flex w-full items-center gap-3 pr-1 pl-8 py-2.5 text-left text-base/6 font-medium text-primary-tail_grids hover:text-primary-mosque sm:py-2 sm:text-sm/5",
    // Leading icon/icon-only
    "data-[slot=icon]:*:size-6 data-[slot=icon]:*:shrink-0 sm:data-[slot=icon]:*:stroke-primary-tail_grids sm:data-[slot=icon]:*:size-[18px]",
    // Trailing icon (down chevron or similar)
    "data-[slot=icon]:last:*:ml-auto data-[slot=icon]:last:*:size-5 sm:data-[slot=icon]:last:*:size-4",
    // Avatar
    "data-[slot=avatar]:*:-m-0.5 data-[slot=avatar]:*:size-7 data-[slot=avatar]:*:[--ring-opacity:10%] sm:data-[slot=avatar]:*:size-6",
    // Hover
    "data-[hover]:bg-primary-primary_light [&>svg>*]:hover:stroke-primary-primary_green",
    // Active
    "data-[active]:bg-primary-primary_light data-[slot=icon]:*:data-[active]:stroke-primary-dark",
    // Current
    "data-[slot=icon]:*:data-[current]:stroke-primary-dark data-[current]:bg-primary-primary_light data-[slot=icon]:*:data-[current]:stroke-primary-primary_green",
    // Dark mode
  ...
  return (
    <span
      className={clsx(
        ...
        "relative",
        current
          ? "border-r-[3px] border-solid border-primary-primary_green"
          : "",
      )}
    >
      {current && (
...
export function SidebarLabel({ className, ...props }) {
  return <span {...props} className={clsx(className, "text-base")} />;
}
